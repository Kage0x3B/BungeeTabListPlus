...
public class VaultGroupInfo implements SortingRule {
	...
	public int compare(Context context, IPlayer player1, IPlayer player2) {
		{
			Optional<Integer> p1Rank = ((TLPlayer) player1).getOpt(MinecraftData.Permissions_PermissionGroupWeight);
			Optional<Integer> p2Rank = ((TLPlayer) player2).getOpt(MinecraftData.Permissions_PermissionGroupWeight);
		...
		{
			Optional<Integer> p1Rank = ((TLPlayer) player1).getOpt(MinecraftData.Permissions_PermissionGroupRank);
			Optional<Integer> p2Rank = ((TLPlayer) player2).getOpt(MinecraftData.Permissions_PermissionGroupRank);
			if(p1Rank.isPresent() || p2Rank.isPresent()) {
				return p1Rank.orElse(Integer.MAX_VALUE) - p2Rank.orElse(Integer.MAX_VALUE);
			}
		}
