...
public class AnimatedComponent extends Component implements Validate {
	...
	public void setComponents(List<Component> components) {
		Preconditions.checkArgument(components.stream().allMatch(Component::hasConstantSize) && components.stream().mapToInt(Component::getSize).distinct().count() == 1, "All elements of an animation are required to have the same size.");
	...
	public static class Instance extends Component.Instance implements Runnable {
		...
		public void activate() {
			super.activate();
			VelocityTabListPlus.getInstance().registerTask(interval / 1000f, this);
		}
		...
		public void deactivate() {
			...
			synchronized(context.get(Context.KEY_TAB_LIST)) {
				VelocityTabListPlus.getInstance().unregisterTask(interval / 1000f, this);
				activeComponent = -1;
			}
		}
