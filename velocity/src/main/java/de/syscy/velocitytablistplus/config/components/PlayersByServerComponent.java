...
package de.syscy.velocitytablistplus.config.components;
...
import de.syscy.velocitytablistplus.playersorting.PlayerSorter;
import de.syscy.velocitytablistplus.tablist.component.ComponentTablistAccess;
import de.syscy.velocitytablistplus.template.IconTemplate;
import de.syscy.velocitytablistplus.template.PingTemplate;
import de.syscy.velocitytablistplus.util.ContextAwareOrdering;
...
public class PlayersByServerComponent extends Component implements Validate {
	...
	public class Instance extends Component.Instance {
		...
		private Map<String, List<TLPlayer>> playersByServer = new LinkedHashMap<>();
		...
		protected Instance(Context context) {
			super(context);
			List<ContextAwareOrdering<Context, String>> list = Stream.of(serverOrder).filter(Objects::nonNull).flatMap(s -> Arrays.stream(s.split(","))).filter((s1) -> !s1.isEmpty()).map(String::toLowerCase).map(this::getServerComparator).filter(Objects::nonNull).collect(Collectors.toList());
			if(list.isEmpty()) {
		...
		private ContextAwareOrdering<Context, String> getServerComparator(String rule) {
			switch(rule) {
				...
				case "online":
					return ContextAwareOrdering.from(Comparator.comparing(server -> VelocityTabListPlus.getInstance().getServerState(server).isOnline(), Comparator.reverseOrder()));
				case "custom":
		...
		public void update1stStep() {
			...
			if(showServers == ServerOptions.ALL) {
				for(RegisteredServer server : VelocityTabListPlus.getInstance().getProxy().getAllServers()) {
					playersByServer.put(server.getServerInfo().getName(), new ArrayList<>());
				}
			}
			if(showServers == ServerOptions.ONLINE) {
				for(RegisteredServer server : VelocityTabListPlus.getInstance().getProxy().getAllServers()) {
					if(VelocityTabListPlus.getInstance().getServerState(server.getServerInfo().getName()).isOnline()) {
						playersByServer.put(server.getServerInfo().getName(), new ArrayList<>());
					}
				}
			}
			...
			for(List<TLPlayer> list : playersByServer.values()) {
		...
		public void setPosition(ComponentTablistAccess cta) {
			if(fillSlotsVertical) {
				...
				super.setPosition(ComponentTablistAccess.createChild(cta, cta.getSize(), index -> (index / rows) + (index % rows) * columns));
			} else {
		...
		public void update2ndStep() {
			...
			if(cta != null) {
				...
				for(int i = 0; i < servers.size() && pos < cta.getSize(); i++) {
					...
					String server = servers.get(i);
					pos = ((pos + columns - 1) / columns) * columns;
					List<TLPlayer> players = playersByServer.get(server);
					// Header
