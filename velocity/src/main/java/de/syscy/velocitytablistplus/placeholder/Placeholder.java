...
package de.syscy.velocitytablistplus.placeholder;
...
import de.syscy.velocitytablistplus.util.Functions;
...
public abstract class Placeholder {
	...
	private static final Map<String, BiFunction<String[], Function<Context, TLPlayer>, Placeholder>> playerPlaceholders = new HashMap<>();
	...
	static {
		...
		playerPlaceholders.put("is_hidden", ofFunction(p -> Boolean.toString(VelocityTabListPlus.isHidden(p))));
		playerPlaceholders.put("gamemode", ofIntData(BTLPVelocityDataKeys.DATA_KEY_GAMEMODE));
		...
		playerPlaceholders.put("redisbungee_server_id", ofStringData(BTLPVelocityDataKeys.DATA_KEY_RedisBungee_ServerId));
		...
		serverPlaceholders.put("tps", (tokens, serverFunction) -> {
			if(tokens.length == 0) {
				return new ServerBoundPlaceholder(serverFunction, server -> {
					Double tps = VelocityTabListPlus.getInstance().getBridge().getServerDataHolder(server).get(MinecraftData.TPS);
		...
		serverPlaceholders.put("online", (tokens, serverFunction) -> {
			return new ServerBoundPlaceholder(serverFunction, serverName -> {
				PingTask serverState = VelocityTabListPlus.getInstance().getServerState(serverName);
			...
			if(tokens.length == 3 && "size".equals(tokens[1])) {
				...
				return new Placeholder() {
					...
					public String evaluate(Context context) {
						List<TLPlayer> players = context.get(Context.KEY_PLAYER_SETS).get(playerSet);
						if(players == null) {
							players = Collections.emptyList();
							VelocityTabListPlus.getInstance().getLogger().info("Missing player set " + playerSet);
						}
	...
	private static Placeholder parsePlayerPlaceholder(String[] tokens, Function<Context, TLPlayer> playerFunction) {
		if(tokens.length == 0) {
			return new PlayerBoundPlaceholder(playerFunction, TLPlayer::getName);
		} else if("server".equals(tokens[0])) {
			...
			VelocityTabListPlus.getInstance().getPlaceholderAPIHook().addMaybePlaceholder(tokens[0]);
			return NULL_PLACEHOLDER;
		}
	}
	...
	private static Placeholder parseServerPlaceholder(String[] tokens, Function<Context, String> serverFunction) {
		if(tokens.length == 0) {
			...
			return new ServerBoundPlaceholder(serverFunction, serverName -> {
				String value = VelocityTabListPlus.getInstance().getBridge().getServerDataHolder(serverName).get(key);
	...
	private static class PlayerBoundPlaceholder extends Placeholder {
		private final Function<Context, TLPlayer> playerFunction;
		private final Function<TLPlayer, String> function;

		@Override
		public String evaluate(Context context) {
			TLPlayer player = playerFunction.apply(context);
	...
	private static class PermissionPlaceholder extends Placeholder {
		...
		private PermissionPlaceholder(Function<Context, TLPlayer> playerFunction, String permission) {
	...
	private static BiFunction<String[], Function<Context, TLPlayer>, Placeholder> ofFunction(Function<TLPlayer, String> function) {
		return (tokens, playerFunction) -> new PlayerBoundPlaceholder(playerFunction, function);
	}

	private static BiFunction<String[], Function<Context, TLPlayer>, Placeholder> ofStringData(DataKey<String> dataKey) {
	...
	private static BiFunction<String[], Function<Context, TLPlayer>, Placeholder> ofIntData(DataKey<Integer> dataKey) {
		return (tokens, playerFunction) -> {
			if(tokens.length == 0) {
	...
	private static BiFunction<String[], Function<Context, TLPlayer>, Placeholder> ofIntFunction(Function<TLPlayer, Integer> function) {
		return (tokens, playerFunction) -> {
			if(tokens.length == 0) {
	...
	private static BiFunction<String[], Function<Context, TLPlayer>, Placeholder> ofDoubleData(DataKey<Double> dataKey) {
		return (tokens, playerFunction) -> {
			if(tokens.length == 0) {
				return new PlayerBoundPlaceholder(playerFunction, player -> player.getOpt(dataKey).map(Object::toString).orElse(""));
			} else {
	...
	private static BiFunction<String[], Function<Context, TLPlayer>, Placeholder> ofFloatData(DataKey<Float> dataKey) {
		return (tokens, playerFunction) -> {
			if(tokens.length == 0) {
				return new PlayerBoundPlaceholder(playerFunction, player -> player.getOpt(dataKey).map(Object::toString).orElse(""));
			} else {
	...
	public static SimpleDateFormat getFormat(String pattern) {
		...
		format.setTimeZone(VelocityTabListPlus.getInstance().getConfig().getTimeZone());
		return format;
	}
}
