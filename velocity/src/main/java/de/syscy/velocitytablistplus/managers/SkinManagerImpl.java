...
public class SkinManagerImpl implements SkinManager {
	...
	public SkinManagerImpl(VelocityTabListPlus plugin, File headsFolder) {
		this.plugin = plugin;
		...
		if(file.exists()) {
			try {
				new BufferedReader(new FileReader(file)).lines().filter(line -> !line.isEmpty()).map(line -> line.split(" ")).forEach(entry -> headCache.put(Head.of(Base64.getDecoder().decode(entry[0])), new Icon(null, Collections.singletonList(new GameProfile.Property("textures", entry[1], entry[2])))));
			} catch(Throwable th) {
	...
	public Icon getIcon(String nameOrUUID) {
		...
		if(!fetchingSkins.contains(nameOrUUID)) {
			...
			plugin.getProxy().getScheduler().buildTask(plugin, new SkinFetchTask(nameOrUUID)).delay(0, TimeUnit.MILLISECONDS).schedule();
		}
		return null;
	}
	...
	private void fetchHeadSkin(File file, byte[] headArray) {
		try {
			...
			if(map.get("state").equals("ERROR")) {
				...
				plugin.getProxy().getScheduler().buildTask(plugin, () -> fetchHeadSkin(file, headArray)).delay(30, TimeUnit.SECONDS).schedule();
			} else if(map.get("state").equals("SUCCESS")) {
				Icon skin = new Icon(null, new GameProfile.Property("textures", (String) map.get("skin"), (String) map.get("signature")));
				...
				VelocityTabListPlus.getInstance().resendTabLists();
				...
				Optional<GameProfile.Property> texturesProperty = skin.getProperties().stream().findAny();
				...
				if(texturesProperty.isPresent()) {
					...
					writer.write(' ');
					writer.write(texturesProperty.get().getValue());
					writer.write(' ');
					writer.write(texturesProperty.get().getSignature());
					writer.newLine();
					writer.close();
				}
			} else {
		...
		plugin.getProxy().getScheduler().buildTask(plugin, () -> fileSkinCache.remove(file, missingSkinTexture)).delay(30, TimeUnit.SECONDS).schedule();
	}
	...
	private String fetchUUID(final String player) {
		...
		try {
			...
			plugin.getLogger().warning("can't fetch uuid for '" + player + "', is not valid name");
		} catch(Throwable e) {
			if(e instanceof IOException && e.getMessage().contains("429")) {
				...
				plugin.getProxy().getScheduler().buildTask(plugin, () -> fetchingSkins.remove(player)).delay(headerField == null ? 300 : Integer.valueOf(headerField), TimeUnit.SECONDS);
			} else {
				...
				plugin.getProxy().getScheduler().buildTask(plugin, () -> fetchingSkins.remove(player)).delay(60, TimeUnit.SECONDS).schedule();
			}
		}
		return null;
	...
	private Icon fetchSkin(String uuid) {
		try {
			uuid = uuid.replace("-", "");
			HttpURLConnection connection = (HttpURLConnection) new URL("https://sessionserver.mojang.com/session/minecraft/profile/" + uuid + "?unsigned=false").
																																										openConnection();
			BufferedReader reader = new BufferedReader(new InputStreamReader(connection.getInputStream(), Charsets.UTF_8));
			SkinProfile skin = gson.fromJson(reader, SkinProfile.class);
			if(skin != null && !skin.properties.isEmpty()) {
				UUID newUuid = UUID.fromString(uuid.substring(0, 8) + "-" + uuid.substring(8, 12) + "-" + uuid.substring(12, 16) + "-" + uuid.substring(16, 20) + "-" + uuid.substring(20, 32));
				return new Icon(newUuid, new GameProfile.Property("textures", skin.properties.get(0).value, skin.properties.get(0).signature));
			}
		} catch(Throwable e) {
			if(e instanceof IOException && e.getMessage().contains("429")) {
				// mojang rate limit; try again later
	...
	public void createIcon(BufferedImage image, Consumer<Icon> callback) {
		...
		try {
			...
			if(map.get("state").equals("SUCCESS")) {
				...
				plugin.getProxy().getScheduler().buildTask(plugin, () -> createIcon(image, callback)).delay(5, TimeUnit.SECONDS).schedule();
			}
		} catch(IOException ex) {
			plugin.getLogger().log(Level.WARNING, "An error occurred while trying to contact skinservice.codecrafter47.dyndns.eu", ex);
			plugin.getProxy().getScheduler().buildTask(plugin, () -> createIcon(image, callback)).delay(1, TimeUnit.MINUTES).schedule();
		}
	}
		...
		public void run() {
			...
			if(uuid != null) {
				...
				if(skin != null) {
					...
					plugin.getProxy().getScheduler().buildTask(plugin, () -> fetchingSkins.remove(nameOrUUID)).delay(1, TimeUnit.MINUTES).schedule();
				}
			}
		}
	}
