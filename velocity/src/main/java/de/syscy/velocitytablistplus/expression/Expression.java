...
public class Expression {
	private static final Map<Token, Integer> OPERATORS = ImmutableMap.<Token, Integer>builder().put(Token.CONCAT_STRING, 25).put(Token.AND, 15).put(Token.OR, 10).put(Token.EQUAL, 5).put(Token.NOT_EQUAL, 5).put(Token.GREATER_THAN, 5).put(Token.LESSER_THAN, 5).put(Token.GREATER_OR_EQUAL_THAN, 5).put(Token.LESSER_OR_EQUAL_THAN, 5).build();
	...
	private abstract static class LiteralPart extends Part {
		@Nonnull @NonNull private final String text;
