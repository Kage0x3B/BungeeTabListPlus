...
import de.syscy.velocitytablistplus.config.components.Component;
...
import de.syscy.velocitytablistplus.playersorting.rules.ViewerInSpectatorModeLast;
...
public class DynamicSizeConfigTablistProvider extends ConfigTablistProvider<DynamicSizeConfig> {
	...
	public DynamicSizeConfigTablistProvider(DynamicSizeConfig config, Context context) {
		...
		this.playerSorter = new PlayerSorter(ImmutableList.<SortingRule>builder().add(new ViewerInSpectatorModeLast()).addAll(config.getPlayerOrder().getRules()).build());
	}
	...
	public synchronized void update() {
		...
		// get players
		List<TLPlayer> players = context.get(Context.KEY_PLAYER_SETS).get(config.getPlayerSet());
		if(players == null) {
			players = Collections.emptyList();
			VelocityTabListPlus.getInstance().getLogger().warning("Missing player set " + config.getPlayerSet());
		}
